...
public class JdbcParkDaoTests extends BaseDaoTests {
    ...
    public void setup() {
        ...
        testPark = new Park(0, "Test Park", LocalDate.parse("2100-01-01"), 99, true);
    }
    ...
    public void getParksByState_with_valid_state_returns_correct_parks() {
        ...
        Assert.assertNotNull("List of parks should not be null", parks);
    ...
    private void assertParksMatch(Park expected, Park actual) {
        Assert.assertEquals("Park id doesn't match", expected.getParkId(), actual.getParkId());
        Assert.assertEquals("Park name doesn't match", expected.getParkName(), actual.getParkName());
        Assert.assertEquals("Date established doesn't match", expected.getDateEstablished(), actual.getDateEstablished());
        Assert.assertEquals("Park area doesn't match", expected.getArea(), actual.getArea(), 0.1);
        Assert.assertEquals("Has camping doesn't match", expected.getHasCamping(), actual.getHasCamping());
    }
