...
public class JdbcCityDaoTests extends BaseDaoTests {
    ...
    private void assertCitiesMatch(City expected, City actual) {
        Assert.assertEquals("City id doesn't match", expected.getCityId(), actual.getCityId());
        Assert.assertEquals("City name doesn't match", expected.getCityName(), actual.getCityName());
        Assert.assertEquals("State abbreviation doesn't match", expected.getStateAbbreviation(), actual.getStateAbbreviation());
        Assert.assertEquals("Population doesn't match", expected.getPopulation(), actual.getPopulation());
        Assert.assertEquals("Area doesn't match", expected.getArea(), actual.getArea(), 0.1);
    }
}
