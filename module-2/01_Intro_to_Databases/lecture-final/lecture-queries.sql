...
--select city_name, population from city; (please capitalize keywords)
...
SELECT city_name, state_abbreviation FROM city WHERE state_abbreviation <> 'CA'; --standard SQL
SELECT city_name, state_abbreviation FROM city WHERE state_abbreviation != 'CA'; -- != available in postgres

-- The names and areas of cities smaller than 25 square kilometers 
SELECT city_name, area, state_abbreviation FROM city WHERE area < 25;

-- The names from all records in the state table that have no assigned census region
SELECT state_name FROM state WHERE census_region IS NULL; --things don't 'equal null', they are null
...
SELECT city_name, area, population FROM city WHERE state_abbreviation = 'CT' OR state_abbreviation = 'ME' OR state_abbreviation = 'MA' OR state_abbreviation = 'NH'; 
--you do not have to type a million OR statements
...
SELECT DISTINCT city_name FROM city; --selecting distinct only selects unique values and does not include repeats
...
--Looking for any city with an e anywhere in it
SELECT city_name FROM city WHERE city_name LIKE '%e%'; 
