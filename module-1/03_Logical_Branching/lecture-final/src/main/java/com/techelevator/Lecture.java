...
public class Lecture {
    ...
    */
    ...
    return true if the if statement evaluates to true.
    */
    ...
    */
    ...
    /*
    12. Now write the above using the Ternary operator ?:. If you're not sure what this is, go read about it in BootcampOS.
    */
    ...
    /*
    13. Write an if/else statement that returns "Fizz" if the parameter is 3, "Buzz" if the parameter is 5
    ...
    */
    public String returnAdultOrMinorAgain(int number) {
        if (number > 17) { //number > 17 || !(number < 18) || switch the return statements || check every number between 0-17
            return "Adult";
        } else {
            return "Minor";
        }
    ...
    */
    public String returnAdultOrMinorOrTeen(int number) {
        ...
        else if (number >= 13) { //could be number >= 13 && number <= 17, but the latter expression is handled by number >=18
            return "Teen";
        }
        else {
            return "Minor"; //child
    ...
    public double returnPizzaCost(char size, int numberOfToppings) {
        ...
        double costOfThePizza = 0;

        // Set its value based on the size. Then add the cost for the toppings and return the total cost
        if(size == 's') //if the size is 's' for a small pizza
        {
            costOfThePizza += SMALL;
        }
        else if(size == 'm')
        {
            costOfThePizza += MEDIUM;
        }
        else if(size == 'l')
        {
            costOfThePizza += LARGE;
        }

        //At this point, we should have the base cost of the pizza depending on what size it is

        double costOfToppings = 0;
        if(numberOfToppings <= 3) //if there are three or less toppings
        {
            costOfToppings = numberOfToppings * UNDER_3_TOPPINGS;
        }
        else {
            costOfToppings = numberOfToppings * OVER_3_TOPPINGS;
        }


        return costOfThePizza + costOfToppings;
    }
