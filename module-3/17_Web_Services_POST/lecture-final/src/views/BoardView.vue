...
export default {
  ...
  methods: {
    deleteBoard() {
      if(
        confirm("Are you really sure you want to delete this board?")
      )
      {
        boardService.deleteBoard(this.$route.params.id)
          .then((response) => {
            this.$store.commit('SET_NOTIFICATION', "Board was successfully deleted");
