...
export default {
  ...
  methods: {
    ...
    saveNewBoard() {
      //make a post request with the newBoard object
      if(this.validateAddForm()) {
        this.isLoading = true;
        boardService.addBoard(this.newBoard)
          .then((response) => {
            this.$store.commit('SET_NOTIFICATION', "Board successfully added!"); 
          this.retrieveBoards();
          //retrieveBoards sets isLoading back to false
          this.resetAddForm();
        })
        .catch((error) => {
          if (error.response) {
            this.$store.commit('SET_NOTIFICATION',
              "Error adding board. Response received was '" + error.response.statusText + "'.");
          } else if (error.request) {
            this.$store.commit('SET_NOTIFICATION', "Error adding board. Server could not be reached.");
          } else {
            this.$store.commit('SET_NOTIFICATION', "Error adding board. Request could not be created.");
          }
        })
      }
    },
